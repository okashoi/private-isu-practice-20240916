...
func makePosts(results []Post, csrfToken string, allComments bool) ([]Post, error) {
	...
	sql := "SELECT `post_id`, COUNT(*) AS `count` FROM `comments` WHERE `post_id` IN ("
	for i, p := range results {
		sql += strconv.Itoa(p.ID)
		if i != len(results)-1 {
			sql += ","
		}
	}
	sql += ") GROUP BY `post_id`"
	err := db.Select(&postCommentCount, sql)
	if err != nil {
		return nil, err
	}

	commentCountMap := make(map[int]int)
	for _, p := range postCommentCount {
		commentCountMap[p.PostID] = p.Count
	}
...
func getIndex(w http.ResponseWriter, r *http.Request) {
	...
	err := db.Select(&results, "SELECT `posts`.`id`, `posts`.`user_id`, `posts`.`body`, `posts`.`mime`, `posts`.`created_at` FROM `posts` INNER JOIN `users` ON `posts`.`user_id` = `users`.`id` WHERE `users`.`del_flg` = 0 ORDER BY `id` DESC LIMIT ?", postsPerPage)
	if err != nil {
		log.Print(err)
		return
	}
...
func getPosts(w http.ResponseWriter, r *http.Request) {
	...
	results := []Post{}
	err = db.Select(&results, "SELECT `posts`.`id`, `posts`.`user_id`, `posts`.`body`, `posts`.`mime`, `posts`.`created_at` FROM `posts` INNER JOIN `users` ON `posts`.`user_id` = `users`.`id` WHERE `created_at` <= ? AND `users`.`del_flg` = 0 ORDER BY `created_at` DESC LIMIT ?", t.Format(ISO8601Format), postsPerPage)
	if err != nil {
		log.Print(err)
		return
	}
...
func getPostsID(w http.ResponseWriter, r *http.Request) {
	...
	results := []Post{}
	err = db.Select(&results, "SELECT `posts`.* FROM `posts` INNER JOIN `users` ON `posts`.`user_id` = `users`.`id` WHERE `posts`.`id` = ? AND `users`.`del_flg` = 0", pid)
	if err != nil {
		log.Print(err)
		return
	}
